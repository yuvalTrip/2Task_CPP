...
// Make sure that p1 and p2 have taken the expected amount of game_cards
    CHECK(p1.cardesTaken() + p2.cardesTaken() == 0);

// Make sure there are in the start of the game still 52 game_cards
    CHECK(p1.cardesTaken() + p2.cardesTaken() + p1.stacksize() + p2.stacksize() == 52);

    CHECK_NOTHROW(game.printStats());// for each player prints basic statistics: win rate, game_cards won, <other stats you want to print>. Also print the draw rate and amount of draws that happand. (draw within a draw counts as 2 draws. )
...
// Make sure there are in the middle of the game still 52 game_cards
    CHECK(p1.cardesTaken() + p2.cardesTaken() + p1.stacksize() + p2.stacksize() == 52);
...
// Make sure there are in the end of the game still 52 game_cards
CHECK(p1.cardesTaken()+ p2.cardesTaken() + p1.stacksize() + p2.stacksize() == 52);
